...
const MonthlyCoffeeTab = () => {
    const [monthlyCoffee, setMonthlyCoffee] = useState<MonthlyCoffeeItem | null>(null);
    ...
    useEffect(() => {
        const fetchMonthlyCoffee = async () => {
            try {
                    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || ""}/api/monthly-coffees/current`);                if (response.ok) {
    ...
    return (
        <div className="bg-background-sub rounded-lg p-4 pt-0 text-gray-0">
            <div className='mb-2 text-center'>
                <h3 className="text-[14px] font-medium mb-1 leading-[20px]">{monthlyCoffee.blend_name || "이달의 커피"}</h3>
            ...
            <div className="flex justify-center gap-2 mt-1">
                <button 
                    ...
                    onClick={() => router.push(`/my-coffee/monthly-coffee/detail?id=${monthlyCoffee.id}`)}
                >
                    자세히 보기
                </button>
            </div>
