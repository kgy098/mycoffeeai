...
const ReviewMain = () => {
  ...
  const isOwnReview = (review: any) => review && user?.user_id && review.user_id === user.user_id;
    ...
    <div className="bg-background">
        ...
        <div className="px-4 pt-2.5 pb-2">
          ...
          <div className="flex justify-between">
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="photoReview" className="text-sm font-bold leading-[16px]">
                포토리뷰
              </label>
            </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {isUserLiked(review.id) ? (
                      ...
                      <ThumbsUp
                        size={16}
                        className={`transition-all duration-300 ${isUserLiked(review.id) ? "text-action-primary" : "text-icon-default"}`}
                      />
                    )}
                  </span>
      ...
      >
        <div className="flex flex-col gap-2">
          <button
            onClick={() => navigateToReviewAnalysys(selectedReview?.blend_id)}
            className="btn-primary"
          >
            커피 취향 분석 보기
          </button>
          {isOwnReview(selectedReview) && (
            <>
              <button
                onClick={handleEdit}
      ...
      {showImageModal && (
        ...
        >
          ...
          >
            ...
            {/* Swiper */}
            <Swiper
              pagination={selectedReviewImages.length > 1 ? { clickable: true } : false}
            ...
            >
              ...
              <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-20 bg-white px-2 py-0.5 rounded-full flex items-center">
                <span className="text-[12px] leading-[18px] font-bold text-action-secondary">
                  ...
                  <span className="text-text-primary">/{String(selectedReviewImages.length)}</span>
                </span>
              </div>
            </Swiper>
          </div>
