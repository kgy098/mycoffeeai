...
export default function AdminBannerEditPage() {
  ...
  useEffect(() => {
    if (!item) return;
    setBlendId(String(item.blend_id));
    if (item.month) {
      const d = item.month.slice(0, 7);
      setMonth(d);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/api/admin/monthly-coffees/${id}`, {
