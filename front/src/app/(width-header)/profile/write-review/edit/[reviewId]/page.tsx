...
const EditReview = () => {
    ...
    // Pre-fill form once review data is loaded
    useEffect(() => {
        if (review) {
            setRating(review.rating || 0);
            setTextareaValue(review.content || "");
            if (review.photo_url) {
                setImages([review.photo_url]);
                setImageCount(1);
            }
        }
    }, [review]);

    const { mutate: updateReview, isPending: isSubmitting } = useQryMutation({
    ...
    const handleSubmit = () => {
        if (!user?.user_id || !rating || !textareaValue.trim()) return;
        updateReview({
    ...
    return (
        <div className="p-4 text-gray-0 flex flex-col h-full">
            ...
            <h3 className="text-sm font-bold leading-[20px] mb-3">
                {review?.blend_name ? `${review.blend_name}에 대한 리뷰를 수정해주세요.` : "리뷰를 수정해주세요."}
            </h3>
            <textarea
                rows={10}
                maxLength={300}
            ...
            >
                {isSubmitting ? "수정 중..." : "리뷰 수정하기"}
            </button>

            <ActionSheet
                isOpen={doneModalIsOpen}
                onClose={() => setDoneModalIsOpen(false)}
            >
